...
export function InteractiveEventMap({ activities, initialZoneId, canRequestPoints = false, activeUserName }: { activities: MapActivity[]; initialZoneId?: MapZoneId; canRequestPoints?: boolean; activeUserName?: string }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)] lg:items-start">
    ...
    {selectedActivity && <ScheduleEventDialog event={selectedActivity} onClose={() => setSelectedActivity(null)} canRequestPoints={canRequestPoints} activeUserName={activeUserName} returnHref={`/mapa?zona=${selectedId}`} />}
  </div>;
}
