...
export function ActivityForm({ action, activity, children }: { action: string; activity?: ScheduleEvent & { dayId: string; active: boolean }; children?: ReactNode }) {
  ...
  return <form action={action} method="post" className="grid gap-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:grid-cols-2 sm:p-8">
    ...
    <label className="grid gap-2 text-sm font-bold">Lugar<input name="location" required defaultValue={activity?.location} className={input} /></label><label className="grid gap-2 text-sm font-bold">Asistencia<select name="required" defaultValue={String(activity?.required ?? false)} className={input}><option value="false">Opcional</option><option value="true">Obligatoria</option></select></label>
