...
export function ContestSubmissionForm({
  ...
  return <form action={action} method="post" encType="multipart/form-data" className="grid gap-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 sm:rounded-3xl sm:p-8">
    <label className="grid gap-2 text-sm font-bold sm:col-span-2">Nombre de {kind === "tapa" ? "la tapa" : "la actuación"}<input name="name" required minLength={2} maxLength={100} className={input} /></label>
    <label className="grid gap-2 text-sm font-bold sm:col-span-2">Descripción<textarea name="description" required minLength={5} maxLength={1000} rows={4} className={input} /></label>
    ...
    <TapaImageInput />
    <div className="grid grid-cols-2 gap-3 border-t border-slate-200 pt-5 sm:col-span-2 sm:flex sm:justify-end"><Link href={backHref} className="grid min-h-12 place-items-center rounded-xl border border-slate-300 px-4 font-bold">Cancelar</Link><button className="min-h-12 rounded-xl bg-[var(--primary)] px-5 font-bold text-white">Publicar</button></div>
  </form>;
}
