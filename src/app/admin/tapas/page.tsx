...
export default async function AdminTapasPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string; message?: string }> }) {
  ...
  return <main className="min-h-screen min-w-0 max-w-full overflow-x-hidden px-3 py-4 sm:px-6 sm:py-12"><div className="mx-auto min-w-0 max-w-7xl">
    <header className="flex items-center justify-between gap-3 border-b border-slate-200 pb-4 sm:pb-5"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--accent)] sm:text-sm">Administración</p><h1 className="mt-1 text-2xl font-black sm:mt-2 sm:text-3xl">Concurso de tapas</h1></div><Link href="/" aria-label="Ir al evento" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-xl text-[var(--primary)] sm:flex sm:w-auto sm:gap-2 sm:border-0 sm:bg-[var(--accent)] sm:px-4 sm:text-sm sm:font-bold sm:text-white"><BsHouse /><span className="hidden sm:inline">Ir al evento</span></Link></header>
    ...
    <div className="flex min-w-0 max-w-full flex-col gap-5 py-4 sm:gap-8 sm:py-10">
      ...
      <section className="min-w-0 max-w-full"><h2 className="text-2xl font-black">Votos emitidos</h2><p className="mt-1 text-slate-600">{votes.length} participantes han votado.</p>{votes.length > 0 && <div className="mt-5 max-w-full overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-xs uppercase text-slate-600"><tr><th className="p-4">Participante</th><th className="p-4">5 puntos</th><th className="p-4">3 puntos</th><th className="p-4">1 punto</th><th className="p-4 text-right">Acción</th></tr></thead><tbody className="divide-y divide-slate-200">{votes.map((vote) => <tr key={vote.userId}><td className="p-4 font-bold">{vote.displayName}</td><td className="p-4">{names.get(vote.firstId)}</td><td className="p-4">{names.get(vote.secondId)}</td><td className="p-4">{names.get(vote.thirdId)}</td><td className="p-4 text-right"><form action={`/admin/tapas/votes/${vote.userId}/delete`} method="post"><button className="rounded-lg border border-red-200 px-3 py-2 text-sm font-bold text-red-700">Borrar voto</button></form></td></tr>)}</tbody></table></div>}</section>
    </div>
  </div></main>;
}
