            ...
            {movements.map((movement) => (
              <li key={movement.id} className="flex items-center gap-4 py-4">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[var(--primary-subtle)] text-xl text-[var(--primary)]">
                  {movement.source === "voucher" ? <BsTicketPerforated aria-hidden="true" /> : movement.source === "game" ? <BsController aria-hidden="true" /> : movement.source === "lottery" ? <BsGift aria-hidden="true" /> : <BsCalendarEvent aria-hidden="true" />}
                </span>
