...
async function toTapa(entity: TapaEntity): Promise<Tapa> {
  ...
  return { id: entity.rowKey, name: entity.name, description: entity.description, participantIds, participantNames: users.map((user) => user.displayName), imageUrl: `/api/tapas/${entity.rowKey}/image`, active: entity.active };
}
...
export async function createTapa(input: { name: string; description: string; participantIds: string[]; image: File }) {
  validate(input);
  const id = randomUUID(); const blobName = `${id}.jpg`;
  await uploadImage(blobName, input.image);
  const entity: TapaEntity = { partitionKey: TAPA_PARTITION, rowKey: id, name: input.name.trim(), description: input.description.trim(), participantIds: JSON.stringify([...new Set(input.participantIds)]), blobName, active: true, createdAt: new Date().toISOString() };
  try { await (await getTableClient()).createEntity(entity); } catch (error) { await (await getContainer()).deleteBlob(blobName).catch(() => undefined); throw error; }
...
export async function updateTapa(id: string, input: { name: string; description: string; participantIds: string[]; active: boolean; image?: File }) {
  ...
  if (input.image?.size) await uploadImage(current.blobName, input.image);
  await client.updateEntity({ ...current, name: input.name.trim(), description: input.description.trim(), participantIds: JSON.stringify([...new Set(input.participantIds)]), active: input.active }, "Merge");
}
...
export async function deleteTapa(id: string) {
  ...
  await client.deleteEntity(TAPA_PARTITION, id); await (await getContainer()).deleteBlob(entity.blobName).catch(() => undefined);
}
